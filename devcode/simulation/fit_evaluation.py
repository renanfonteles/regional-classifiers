...
from devcode.utils.hyperparameters import HyperOptimization, som_params
...
def _fit_and_evaluate_regional_ols(case, X_tr_norm, y_train, X_ts_norm, y_test, hps_cases):
    ...
    # Load information about SOM training already generated by Regional LSSVM
    rlssvm_params = HyperOptimization.load_optimal_hyperparams(dataset_name, random_state, model_name="R-LSSVM")

    som_tr = rlssvm_params[-1]
    ...
    rlm.fit(X=X_tr_norm, Y=y_train, SOM_params=som_params, cluster_params=best_region_params)

    cm_tr, cm_ts = _evaluate_train_test_sets(X_tr_norm, y_train, X_ts_norm, y_test, model=rlm, round_flag=True)

    header     = ["k_opt"]
    result_row = ["No eigenvalues", "No eigenvalues", cm_tr, cm_ts, best_region_params["n_clusters"]]

    for k in k_values:
        kth_params = {'n_clusters': k, 'n_init': 10, 'init': 'random'}

        linearModel = LinearRegression()

        rlm = RegionalModel(som_tr, linearModel)
        rlm.fit(X=X_tr_norm, Y=y_train, SOM_params=som_params, cluster_params=kth_params)

        cm_tr, cm_ts = _evaluate_train_test_sets(X_tr_norm, y_train, X_ts_norm, y_test, model=rlm, round_flag=True)

        result_row += [cm_tr, cm_ts]     # list(som_params.values()) +
        header     += [f"k={k}_tr", f"k={k}_ts"]

    return {"result_row": result_row, "header": header}
