...
from devcode.analysis.clustering import cluster_val_metrics, organize_cluster_metrics, find_optimal_clusters, \
    RegionalUtils, regional_cluster_val_metrics
...
from devcode.simulation.hyperparameters import HyperOptimization, default_som_params
...
def _fit_and_evaluate_regional_ols(case, X_tr_norm, y_train, X_ts_norm, y_test, hps_cases):
    """ Regional Ordinary Least Square (R-OLS) """
    dataset_name, random_state = case["dataset_name"], case["random_state"]

    # Fit the model on best hyperparameters
    opt_params = HyperOptimization.search_best_params("R-OLS", HyperOptimization.search_regional_ols_best_hp,
                                                      dataset_name, random_state, X_tr_norm, y_train, hps_cases)

    best_k, som_tr = opt_params

    C = som_tr.nColumns ** 2  # number of SOM neurons in the 2D grid

    k_values = [i for i in range(1, ceil(np.sqrt(C)))]  # 2 to sqrt(C)
    ...
    best_region_params = RegionalUtils.search_optimal_region_params("R-OLS", dataset_name, random_state, cluster_params,
                                                                    som_tr.neurons)

    linearModel = LinearRegression()

    rlm = RegionalModel(som_tr, linearModel)
    rlm.fit(X=X_tr_norm, Y=y_train, SOM_params=default_som_params, cluster_params=best_region_params)

    cm_tr, cm_ts = _evaluate_train_test_sets(X_tr_norm, y_train, X_ts_norm, y_test, model=rlm, round_flag=True)

    result_row = ["No eigenvalues", "No eigenvalues", cm_tr, cm_ts]
...
def run_simulation(model_name, datasets, header, case, scale_type, test_size, hps_cases, pipeline_func,
    ...
    pipeline_output = pipeline_func(case, X_tr_norm, y_train, X_ts_norm, y_test, hps_cases)

    if isinstance(pipeline_output, dict):
        result_row         = pipeline_output["result_row"]
        _additional_header = pipeline_output["header"]
        final_header = header + _additional_header
    else:
        result_row = pipeline_output
        final_header = header

    result_row = [dataset_name, random_state] + result_row

    _save_results(result_row, simulation_file.__str__(), final_header)
...
def eval_GOLS(datasets, simulation_file, header, scale_type, test_size, hps_cases, case):
...
def eval_ROLS(datasets, simulation_file, header, scale_type, test_size, hps_cases, case):
    pipeline_func = _fit_and_evaluate_regional_ols
